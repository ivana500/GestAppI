...
if (isset($_GET['identifiant'])) {
    $a = 1;
        ...
        $code = $_GET['identifiant'];
         $sql = "SELECT * FROM matieres WHERE identifiant ='$code'";
         $conn = getConnection();
        ...
        $sql = "UPDATE matieres SET intitule ='$intitule' WHERE identifiant = '$code'";
        $conn = getConnection();
        $result = $conn->query($sql);
        $conn->close();
        if($result){
           redirection("listMatiere.php");
        }else{
            echo 'error';
        }
            ...
            switch ($action) {
                ...
                case 'deleteMatiere':
                    $code = $_POST['identifiant'];
                         $del = "DELETE FROM matieres WHERE identifiant = '$code'";
                         $con = getConnection();
                         $delet = $con->query($del);
        ...
        <div class="card border-primary mb-4 rounded-3">
            ...
            <form id="ajoutEtudiantForm" method="post">
                    <div class="mb-3">
                        ...
                        <input type="text" name="code" class="form-control" id="code" value="<?php if($a == 1) echo $ligne['identifiant']?>" placeholder="Entrez le code de la filiere" required maxlength="6" pattern="[A-Za-z0-9]{1,6}" title="Le code doit contenir 1 à 6 caractères alphanumériques.">
